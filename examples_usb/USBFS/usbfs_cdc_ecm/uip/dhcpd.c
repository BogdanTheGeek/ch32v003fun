...
struct dhcpd_msg
{
	...
	u8_t siaddr[4]; // 若 client 需要透过网络开机，从 server 送出之 DHCP
	                // OFFER、DHCPACK、DHCPNACK封包中，此栏填写开机程序代码所在 server 之地址。
...
static u8_t *add_end1( u8_t *optptr )
{
	char pend[27] = { 0x1c, 0x04, 0xc0, 0xa8, 0x01, 0xff, 0x03, 0x04, 0xc0, 0xa8, 0x01, 0x01, 0x06, 0x04, 0xc0, 0xa8,
		0x01, 0x01, 0x0f, 0x03, 0x6c, 0x61, 0x6e, 0xff, 0x00, 0x00, 0x00 };
...
int find_vaild_hwaddr( NODE *head, char *HwAddress )
{
	NODE *p;
	p = head;
	while ( ( p->next ) && ( strcmp( p->next->hwaddr, HwAddress ) ) ) p = p->next;
	if ( p->next )
	{
		printf( "Found match ipaddr: %d.%d.%d.%d\n", p->next->ipaddr.addr & 0x000000ff,
			( p->next->ipaddr.addr & 0x0000ff00 ) >> 8, ( p->next->ipaddr.addr & 0x00ff0000 ) >> 16,
			( p->next->ipaddr.addr & 0xff000000 ) >> 24 );
		return TRUE;
	}
	else
	{
...
static u8_t parse_dhcp_options( u8_t *optptr, int len )
{
	...
	while ( optptr < end )
	{
		switch ( *optptr )
		{
			case DHCP_OPTION_SUBNET_MASK: memcpy( s.netmask, optptr + 2, 4 ); break;
			case DHCP_OPTION_ROUTER: memcpy( s.default_router, optptr + 2, 4 ); break;
			case DHCP_OPTION_DNS_SERVER: memcpy( s.dnsaddr, optptr + 2, 4 ); break;
			case DHCP_OPTION_MSG_TYPE: type = *( optptr + 2 ); break;
			case DHCP_OPTION_SERVER_ID: memcpy( s.serverid, optptr + 2, 4 ); break;
			case DHCP_OPTION_LEASE_TIME: memcpy( s.lease_time, optptr + 2, 4 ); break;
			case DHCP_OPTION_REQ_IPADDR: memcpy( s.ipaddr, optptr + 2, 4 ); break;
			case DHCP_OPTION_END: return type;
		}
...
static int dhcpd_relay_offer( NODE *head )
{
	...
	p = head;
	struct dhcpd_msg *m = (struct dhcpd_msg *)uip_appdata;
	while ( ( p->next ) && ( strcmp( p->next->hwaddr, m->chaddr ) ) ) p = p->next;
	if ( p->next )
	{
		addr = 0x0201a8c0;
...
int dhcpd_init( void )
{
   ...
   u16_t addr[2];
	...
	s.conn = uip_udp_new( addr, DHCPC_CLIENT_PORT );

	if ( s.conn != NULL )
	{
      printf("dhcpd_init: bind dhcp server port %d\n", DHCPC_SERVER_PORT);
		// uip_udp_bind( s.conn, HTONS( DHCPC_SERVER_PORT ) );
      s.conn->lport = HTONS( DHCPC_SERVER_PORT );
	}
	return s.conn != NULL ? 0 : -1;
}
